...
const initialState = {
  articleType: ARTICLE_TYPES.TOP_HEADLINES,
  searchResults: [],
  activeSources: [],
}
const actionTypes = {
  ...
  CLEAR_SEARCH: "SOURCE/CLEAR_SEARCH",
}
...
export function fetchSources() {
  ...
  return dispatch => {
    ...
    axios
      .get(url)
      .then(resp =>
        dispatch(actionCreators.sourceLoadSuccess(resp.data.sources))
      )
      .then(sources =>
        dispatch(actionCreators.updateDetails(filter(sources.sources)))
      )
...
export function dropSource(source) {
  return (dispatch, getState) => {
    const activeSources = getState().sources.activeSources
...
export function clearSearch() {
  return dispatch => dispatch(actionCreators.clearSearch())
}
