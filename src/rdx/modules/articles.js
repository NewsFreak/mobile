...
const actionTypes = {
  ...
  RESET_PAGINATION: "ARTICLE/RESET_PAGINATION",
}
...
export const selectors = {
  ...
  getTotalPages: state =>
    Math.floor(state.articles.results / state.articles.pageSize),
}
...
export function fetchArticles() {
  return (dispatch, getState) => {
    const { articleType, activeSources } = getState().sources
    const { page, pageSize } = getState().articles
    const url = getState().sources.focusedSource
      ? urlBuilder(
          getState().sources.focusedSource,
          articleType,
          page,
          pageSize
        )
      : urlBuilder(activeSources, articleType, page, pageSize)
...
export function refreshArticles() {
  return (dispatch, getState) => {
    ...
    const url = getState().sources.focusedSource
      ...
      : urlBuilder(
          getState().sources.activeSources,
          articleType,
          page,
          pageSize
        )
...
export function resetPagination() {
  return dispatch => {
    dispatch(actionCreators.resetPagination())
  }
}
